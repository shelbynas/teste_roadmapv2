...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btnGerar").addEventListener("click", gerarTrilha);
});
...
async function gerarTrilha() { 
  ...
  roadmapDiv.innerHTML = "✨ Gerando sua trilha de estudos..."; 
  ...
  try {
    ...
    // INJETA O BOTÃO DE CHAT ANTES DA GRADE DA TRILHA
    roadmapDiv.innerHTML = `
        <button class="btn-chat-duvidas" onclick="abrirChatDuvidas()">
            💬 Tirar Dúvidas com o Pato IA
        </button>
    `;

    // Cria a área do grid para as etapas
    const gridDiv = document.createElement("div");
    gridDiv.className = "roadmap-grid";
    ...
    etapas.forEach(etapa => {
      ...
      gridDiv.appendChild(blocoDiv);
    });

    roadmapDiv.appendChild(gridDiv);


  } catch (err) {
    console.error("Erro:", err);
    roadmapDiv.innerHTML = `⚠️ Erro ao gerar trilha. Causa: ${err.message}.`;
  }
}
